...
后续开发与发布以 `ashare-compass` 为源码入口。Python 包名、命令及发行包名保持兼容；部署目录和服务名称可以独立迁移，其当前值及切换记录保存在 `.local/OPERATIONS.md`。新维护任务应先读取私有记录并核对现场配置，不得依据历史示例恢复已下线的部署。
...
发布前从当前源码建立明确的文件清单和哈希，核对实际目标并备份需要替换的文件；更新后验证维护脚本、服务、健康接口及报告状态，失败按对应发布记录回退。常规升级保留服务器既有 `.env`、数据、报告、预算与尝试记录，避免对整个运行目录做镜像覆盖。用户明确要求清理报告或分析缓存时按授权范围执行；预算与尝试记录仍须保留，防止迁移重置调用额度。网页可访问与最新日报生成成功是两项独立检查。
...
`.local/deployment.json` 不包含服务前缀或 Nginx 文件名字段。当前 `scripts/linux_services.py` 固定使用 `ashare-daily-research-*` unit 名称；`scripts/nginx_intranet.py` 固定使用旧名称的 vhost、日志和配置标记。更改 `project_root` 不会改变这些名称。对于已经重命名的部署，不得直接使用这些工具执行安装、启停、更新或删除等管理动作；离线预览仅展示现有模板，不能作为已迁移实例的安装配置。维护应采用与私有记录和现场配置匹配的操作。
...
`partial` 表示仍有未备份的引用或其他缺口，不等于完整恢复。迁移时要保留模型预算和尝试记录，不以重建空账本重置当天调用额度。需要保留的旧报告和快照按原字节保存，不能为修改路径而重写历史证据。
